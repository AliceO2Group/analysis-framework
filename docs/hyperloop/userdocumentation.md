...
## <a name="local-train"></a>Reproducing a train run on a local machine
 ...
 In order to do so, you need two general prerequisites:
   * Download the `run_train.sh` script from: TODO. Say this is placed in a folder `/my/path/`.
   * Make sure `jq` is installed on your system. Type `jq` on the command prompt. If you get a command not found error you have to install it. This package is a system package on most systems (it has nothing to do with ALICE). Use Google if you need instructions for your specific operation system.
...
```
 ...
 ```bash
 /path/to/my/run_train.sh --skip-perf
 ```
