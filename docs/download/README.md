...
* You can also click on processing progress and then follow the output directories to download a larger AO2D.root (Note that those are in the folder `AOD`). All this is only suitable for manual downloading of a few files. For everything else see the previous item.
